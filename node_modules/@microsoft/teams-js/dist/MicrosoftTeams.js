...
Object.defineProperty(exports, "__esModule", { value: true });
var appInitialization_1 = __webpack_require__(26);
...
var tasks_1 = __webpack_require__(29);
...
var interfaces_2 = __webpack_require__(19);
var communication_1 = __webpack_require__(0);
var handlers_1 = __webpack_require__(3);
var constants_2 = __webpack_require__(4);
var media;
(function (media) {
    ...
    var MediaController = /** @class */ (function () {
        ...
        MediaController.prototype.notifyEventToHost = function (mediaEvent, callback) {
            internalAPIs_1.ensureInitialized(constants_1.FrameContexts.content, constants_1.FrameContexts.task);
            var err = mediaUtil_1.isApiSupportedOnMobile(constants_2.nonFullScreenVideoModeAPISupportVersion);
            if (err) {
                if (callback) {
                    callback(err);
                }
            ...
            communication_1.sendMessageToParent('mediaController', [params], function (err) {
                console.log("mediaController sendMessageToParent with mediaType : " + params.mediaType + ", mediaControllerEvent : " + params.mediaControllerEvent);
                if (callback) {
                    callback(err);
                }
            });
        ...
        MediaController.prototype.stop = function (callback) {
            this.notifyEventToHost(interfaces_2.MediaControllerEvent.StopRecording, callback);
        };
    ...
    var VideoController = /** @class */ (function (_super) {
        ...
        VideoController.prototype.notifyEventToApp = function (mediaEvent) {
            console.log("entered notifyEventToApp = TYPE " + typeof mediaEvent + " " + this.controllerCallback + " " + typeof interfaces_2.MediaControllerEvent.StartRecording);
            if (!this.controllerCallback) {
                console.log("early return callbacknot registered in video controller");
                // Early return as app has not registered with the callback
                return;
            }
            switch (mediaEvent) {
                case interfaces_2.MediaControllerEvent.StartRecording:
                    console.log("case MediaControllerEvent.StartRecording");
                    if (this.controllerCallback.onRecordingStarted) {
                        console.log("notified the app about recording started");
                        this.controllerCallback.onRecordingStarted();
                        return;
                    }
                default:
                    console.log("Entered default case - " + mediaEvent);
            }
        };
    ...
    function selectMedia(mediaInputs, callback) {
        ...
        if (mediaUtil_1.isMediaCallForVideoAndImageInputs(mediaInputs)) {
            var err = mediaUtil_1.isApiSupportedOnMobile(constants_2.videoAndImageMediaAPISupportVersion);
            if (err) {
                callback(err, null);
                return;
            }
        ...
        console.log('Select Media sendMessageToParent');
        ...
        communication_1.sendMessageToParent('selectMedia', params, function (err, localAttachments, mediaEvent) {
            ...
            if (mediaEvent) {
                console.log("Select Media MediaControllerEvent " + mediaEvent);
                if (mediaUtil_1.isVideoControllerRegistered(mediaInputs)) {
                    console.log("isVideoControllerRegistered = true");
                    mediaInputs.videoProps.videoController.notifyEventToApp(mediaEvent);
                }
                console.log("Early return from partial response of Select Media MediaControllerEvent");
                return;
            }
            // Media Attachments are final response to selectMedia
            if (!localAttachments) {
                console.log("Select Media error " + err.errorCode + " - " + err.message);
                callback(err, null);
                return;
            }
            var mediaArray = [];
...
function isApiSupportedOnMobile(requiredVersion) {
    ...
    else if (!internalAPIs_1.isAPISupportedByPlatform(requiredVersion)) {
        var oldPlatformError = { errorCode: interfaces_1.ErrorCode.OLD_PLATFORM };
...
(function (MediaControllerEvent) {
    MediaControllerEvent[MediaControllerEvent["StartPreview"] = 1] = "StartPreview";
    MediaControllerEvent[MediaControllerEvent["StartRecording"] = 2] = "StartRecording";
    MediaControllerEvent[MediaControllerEvent["PauseRecording"] = 3] = "PauseRecording";
    MediaControllerEvent[MediaControllerEvent["ResumeRecording"] = 4] = "ResumeRecording";
    MediaControllerEvent[MediaControllerEvent["StopRecording"] = 5] = "StopRecording";
})(MediaControllerEvent = exports.MediaControllerEvent || (exports.MediaControllerEvent = {}));
...
Object.defineProperty(exports, "__esModule", { value: true });
var bot_1 = __webpack_require__(22);
